...
struct ModuleAttrs {
    ...
    // This constructor is used when putting together the attributes of a user defined module or kronk file
    ModuleAttrs(std::unique_ptr<llvm::Module> ThisModule)
        : TheModule(std::move(ThisModule)) {}
    
    // This constructor is used when putting together the attributes of a kronk stdlib/rtModule
    ModuleAttrs(std::string&& rtModule)
        : rtModule(std::move(rtModule)) {}
};
