...
// A stack whose top element holds the name of the current module being compiled, whose bottom element holds
// the name of the main module, and whose other elements are modules whose compilation has been suspended 
// just like main module.
std::vector<std::unique_ptr<SuspendedModuleState>> SuspendedModuleStack;

// maps a moduleId of a module to the primordial moduleId that was used to compile that module. This prevents
// multiple compilation of the same module with different moduleIds. 
std::unordered_map<std::string, std::string> DuplicateModuleMap;
// maps a filename ( the absolute file path ) to the moduleId used to compile it (this moduleId is referred) 
// to as the primordial moduleId
...
const std::unordered_map<std::string, std::string> DirectFunctions = {
    {"afficher", "io"}
    };
