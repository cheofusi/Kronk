...
bool isBool(Type* ty) { return ty->isIntegerTy(1); }

bool isBool(Value* v) { return isBool(v->getType()); }


bool isReel(Type* ty) { return ty->isDoubleTy(); }

bool isReel(Value* v) { return isReel(v->getType()); }
...
StructType* isEnttyPtr(Value* v) { return isEnttyPtr(v->getType()); }
...
bool isListePtr(Value* v) { return isListePtr(v->getType()); }
...
bool isStringPtr(Value* v) { return isStringPtr(v->getType()); }
...
std::string typestr(Value* v) { return typestr(v->getType()); }
