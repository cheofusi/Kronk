...
Value* IncludeStmt::codegen() {
	...
	if (not rtModule.empty()) {
		// the included module is a stdlib/rt module, hence we don't have any compilation to do because
		// the stdlib is already compiled. So we construct an Attr::ModuleAttrs object whose only
		// non-null data member is the actual name of the runtime module = rtModule
