...
Value* Declr::codegen() {
	...
	if (typeTy->isStructTy()) {
		...
		if (enttyTy->isLiteral()) {
			...
			auto dataPtr = Attr::Builder.CreateAlloca(
			    enttyTy->getTypeAtIndex(1)->getPointerElementType(), listSizeV);
...
Value* InitDeclr::codegen() {
	...
	// An Expression node on emitting ir always returns one of two types: a value holding an i1, i64, or
	// double; or a pointer holding the address of some entity, since the default ctx for all expressions
	// is Load
	if (auto rvalueTy = types::isEnttyPtr(rvalue)) {
		...
		// if it was just constructed, say be a list splice, anonymous list or entity constructor then it
		// will be in the HeapAllocas record but not in the SymbolTable. We only need check the latter
		// condition and if true point name to it. If not then we have to copy rvalue

		auto it = std::find_if(
		    Attr::ScopeStack.back()->SymbolTable.begin(), Attr::ScopeStack.back()->SymbolTable.end(),
		...
		// rvalue either holds the address of a primitive type, or is the result of some operation that
		// returns
		/// a primitve type
