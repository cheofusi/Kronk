...
static const std::unordered_map<std::string, uint8_t> OpIds = { { "non", 0 }, { "~", 1 },
...
Value* Assignment::codegen() {
	...
	auto lhsV =
	    lhs->codegen();  // always returns a pointer type ex i1*, double*, { double }**, { double }*
	...
	if (lhsTy->isStructTy() or types::isEnttyPtr(lhsTy)) {
		...
		// this takes care of the special case where rhs is an Identifier Node that codegens an entity
		// ptr which ignores the ctx, giving types like { double }* instead of { double }**. Any call to
		// lhsTy->isStrucTy() inside this if block is dealing with this exception
	...
	if (lhsTy->isIntegerTy(8)) {
		...
		if (not types::isStringPtr(rhsV))
			irGenAide::LogCodeGenError(
			    "Trying to replace a character of a string with a non-string value");
...
Value* UnaryExpr::codegen() {
	...
	if (types::isReel(rhsV)) {
		switch (OpId) {
			...
			default:
				irGenAide::LogCodeGenError("Undefined unary operator << " + Op +
				                           " >> for a real number");
		}
	}
...
Value* BinaryExpr::codegen() {
	...
	if (types::isBool(lhsV) and types::isBool(rhsV)) {
		switch (OpId) {
			...
			default:
				irGenAide::LogCodeGenError("Undefined binary operator << " + Op +
