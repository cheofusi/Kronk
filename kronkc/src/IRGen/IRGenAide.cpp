...
void LogCodeGenError(std::string errMsg) {
	outs() << "Code Generation Error in " << names::getModuleFile().filename() << '\n'
	       << "[Line " << (Attr::CurrentLexerLine - Attr::IRGenLineOffset - 1) << "]: " << errMsg
	       << '\n';
...
Value* getConstantInt(int value) { return ConstantInt::get(Attr::Builder.getInt64Ty(), value, true); }
...
void emitMemcpy(Value* dst, Value* src, Value* numElemstoCopy) {
	auto dataTypeSize = Attr::ThisModule->getDataLayout()
	                        .getTypeAllocSize(src->getType()->getPointerElementType())
	                        .getFixedSize();
	...
	Attr::Builder.CreateMemCpy(dst, dst->getPointerAlignment(Attr::ThisModule->getDataLayout()), src,
	                           src->getPointerAlignment(Attr::ThisModule->getDataLayout()),
	                           numBytestoCopyV);
}
...
void deepCopy(Value* enttyPtr) {
	...
	// recursively copy all members of this entity that are also entities, since these members are
	// actuallly pointers
...
void fillUpListEntty(Value* listPtr, std::vector<Value*> members, std::vector<Value*> dataValues) {
	...
	if (dataValues.size()) {
		for (int i = 0; i < dataValues.size(); ++i) {
			Attr::Builder.CreateStore(
			    dataValues[i], irGenAide::getGEPAt(members[1], irGenAide::getConstantInt(i), true));
		}
	}
}
