...
std::string mangleNameAsLocalFunction(const std::string& name) {
    ...
    auto moduleId = Attr::ThisModule->getModuleIdentifier(); 
    auto mname = "_Z" + std::to_string(moduleId.size()) + moduleId
...
std::string mangleIncludeId(const std::string& includeId) {
    ...
    // identifier ex. in main.krk we can have `inclu mylib as l`. The mylib file/module will compiled with
    // the moduleId `main_l`, given of course that main.krk 's moduleId is `main`
...
std::pair<std::string, std::string> demangleName(const std::string& name) {
    // demangles name by spliting it into 2 components; the prefix, which is a (supposed) moduleId in which 
    // the suffix, which is either an entity type or function identifier, is defined.
    // The function then tries replacing the prefix with its primordial moduleId.
...
std::optional<StructType*> EntityType(const std::string& name) {
    ...
    // moduleId either refers to the current module or is a dep or the current module. The caller must have checked
...
std::optional<llvm::Function*> Function(const std::string& name) {
    ...
    if(Attr::ThisModule->getModuleIdentifier() == moduleId) {
        ...
        // moduleId is a dep of the current module. The caller must have checked that before calling this function
        // moduleId may not be primordial but in that case the call to demangleName() must have fixed that. Also the
        // module it refers to must have already been compiled because the compile driver doesn't allow circular deps.
        ...
        if(moduleAttrs->rtModule.empty()) {
            ...
            // kronkrt module. We need to mangle the symbol before we look for it. ex in the runtime module `math`,
            // the function `sin` is defined as `_kmath_sin`
...
fs::path getModuleFile(const std::string& moduleId) {
    ...
    auto it = std::find_if(
                            Attr::FileToModuleIdMap.begin(),
                            Attr::FileToModuleIdMap.end(),
                            [mId](const auto& entry)
                                {
...
bool createsCircularDep(const std::string& moduleId) {
    // returns false if moduleId doesn't create circular dependency and true otherwise. If A.krk includes B.krk, 
    // we can denote the relation as A <-- B. Then examples of circular deps would be
    //      A <-- A  
    //      A <-- B <-- A
    //      A <-- B <-- C < -- A
    //      A <-- B <-- C < -- B

    // This happens when we try to include a module whose compilation state is pending i.e it is somewhere in
    // the module stack.
