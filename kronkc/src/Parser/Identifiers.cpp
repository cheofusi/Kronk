...
std::unique_ptr<Identifier> ParserImpl::ParseForeignSymbol() {
	...
	// And since the caller admits foreign symbols in whatever context it is parsing, this method is
	// called to pass the rest i.e '::Point'.

	moveToNextToken();  // eat first :
	if (not isCurrTokenValue(':')) LogError("Expected two semi colons, got one");
...
std::unique_ptr<TypeId> ParserImpl::ParseTypeId() {
	...
	if (id->name == "liste") {
		if (not isCurrTokenValue('(')) LogError("Expected '(' after liste keyword");
		moveToNextToken();  // eat (

		if (currentToken != Token::IDENTIFIER) LogError("Expected identifer for the list type");
	...
	// this part is tricky. consider the statement ` soit lst: liste(km::Point) `. In this case id=km at
	// this point when it rather refers to a module. so we have to look after it.
	...
	if (isCurrTokenValue(':')) {
		...
		LogError("<< " + id2->name + " >> is not an entity type in the module << " +
	...
	LogError("The type << " + id->name + " >> doesn't exist in the module << " +
...
std::unique_ptr<Node> ParserImpl::ParseIdOp() {
	...
	if (isCurrTokenValue(':')) {
		...
		if (it == Attr::Dependencies.end()) {
			LogError("<< " + id->name + " does not identify any included module ");
		}
		...
		LogError("<< " + id2->name + " >> is neither an entity type nor a function in the module << " +
		         names::demangleNameForErrMsg(nameAsForeignType).first + " >>");
	}
