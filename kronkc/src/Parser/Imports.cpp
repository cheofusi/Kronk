...
std::unique_ptr<IncludeStmt> ParserImpl::ParseIncludeStmt() {
	...
	if (isCurrTokenValue(':')) {
		...
		if (currentToken != Token::IDENTIFIER) LogError("Expected identifier for module to be included");

		while (true) {
			auto id = ParseIdentifier();
			userModule.push_back(id->name);

			if (not isCurrTokenValue('.')) break;

			moveToNextToken();  // eat :
			if (currentToken != Token::IDENTIFIER) LogError("Expected identifier after the semi colon");
		}
	}
	...
	if (currentToken == Token::IDENTIFIER) {
		...
		if (alias_keyword->name != "as") LogError("Expected alias keyword 'as' ");
