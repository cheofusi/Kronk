...
std::unique_ptr<EntityDefn> ParserImpl::ParseEntityDefn() {
	...
	if (not isCurrTokenValue('{')) LogError("Expected '{' to follow << " + enttyTypeId->name + " >>");

	moveToNextToken(true);  // eat '{' and skip all subsequent new lines

	if (isCurrTokenValue('}')) LogError("kronk cannot create an entity with zero fields");

	std::vector<std::string> memberNames;
	std::vector<std::unique_ptr<TypeId>> memberTypeIds;
	while (true) {
		if (currentToken != Token::IDENTIFIER) LogError("Expected an identifer string for a field");
		auto fieldName = ParseIdentifier();

		if (not isCurrTokenValue(':')) LogError("Expected ':' after << " + fieldName->name + " >>");
		moveToNextToken();  // eat :

		if (currentToken != Token::IDENTIFIER) LogError("Expected an identifier string for a type ");
	...
	if (currentToken == Token::IDENTIFIER) {
		...
		while (i < entitySignature.size()) {
			...
			if (it == entitySignature.end())
				LogError("<< " + field->name + " >>" + " is not a valid field of << " +
				         names::demangleName(enttyTypeId).second + " >>");

			// arg is a valid field. Now we parse value for this field
			if (not isCurrKronkOperator("=")) LogError("Expected '=' after << " + field->name + " >>");
			...
			moveToNextToken();  // eat ','
			if (currentToken != Token::IDENTIFIER) LogError("Expected a field after ',' ");
			...
			if (i == entitySignature.size())
				LogError("Constructor for the entity type << " +
				         names::demangleName(enttyTypeId).second + " >> is too long");
		}
...
std::unique_ptr<EntityOperation> ParserImpl::ParseEntityOperation(std::unique_ptr<Node> entty) {
	if (currentToken != Token::IDENTIFIER) LogError("Expected alphanumeric string for a field");
