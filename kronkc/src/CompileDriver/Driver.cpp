...
std::string CompileDriver::compileInputFile(std::string&& moduleId) {
	...
	// construct the module for this file. If the caller didn't provide a name for the module, use the
	// file name without the extension
	if (moduleId.empty()) moduleId = inputFile.stem().string();

	// check if the file has already been referenced (i.e it has already been compiled or it is somewhere
	// in the Attr::ModuleStack)
	...
	if (not not_referenced) {  // a double negative...I know
		// file has been referenced. so just point moduleId to the primordial moduleId used to reference
		// the file and return. But before that make sure we're not dealing with a circular dependency ex
		// in A.krk we have `inclu B`, and in B.krk we have `inclu A. This is a fatal fucking error
	...
	auto mainFn =
	    Function::Create(mainFnTy, llvm::GlobalValue::ExternalLinkage, "main", Attr::ThisModule.get());
	...
	Attr::Builder.CreateRet(
	    Attr::Builder.getInt32(0));  // On termination our program always returns zero
...
void CompileDriver::registerModule() {
	...
	Attr::ModuleMap.insert(
	    std::make_pair(moduleAttrs->TheModule->getModuleIdentifier(), std::move(moduleAttrs)));
}
...
bool CompileDriver::inputFileExists() {
	if (inputFile.string().find("/") != std::string::npos) {
		...
		for (auto& file : fs::recursive_directory_iterator(fs::current_path(),
		                                                   fs::directory_options(fileSearchOptions))) {
...
void CompileDriver::LogImportError(std::string errMsg) {
	outs() << "Module Include Error in " << names::getModuleFile().filename() << '\n'
