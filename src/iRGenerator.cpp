...
Value* Declr::codegen() {
    auto typeT = type->typegen();
    AllocaInst* alloc;

    if(typeT->isStructTy()) {
        auto enttypeT = llvm::cast<StructType>(typeT);
        alloc = builder.CreateAlloca(enttypeT);

        if(enttypeT->isLiteral()) {
            // We're declaring a liste with zero elements
            
            auto listSizeV = iRGenHelpers::getLLVMConstantInt(0);
            auto dataPtr = builder.CreateAlloca(enttypeT->getTypeAtIndex(1)->getPointerElementType(), listSizeV);
        ...
        alloc = builder.CreateAlloca(typeT);
    }
...
Value* EntityOperation::codegen() {
    ...
    auto enttypeT = iRGenHelpers::isEnttyPtr(enttyPtr);
    ...
    if (iRGenHelpers::isListePtr(enttyPtr)) {
        ...
        enttypeStr = std::string(enttypeT->getName());
...
Value* Assignment::codegen() {
    ...
    auto lhsT = lhsV->getType()->getPointerElementType();
    auto rhsT = rhsV->getType();
    

    if(lhsT->isStructTy() or iRGenHelpers::isEnttyPtr(lhsT)) {
        ...
        // which ignores the ctx, giving types like { i64 }* instead of { i64 }**. Any call to lhsT->isStrucTy() inside
        // this if block is dealing with this exception
        auto lvalueT = (lhsT->isStructTy()) ? lhsV->getType() : lhsT;

        if(not iRGenHelpers::isEnttyPtr(rhsV)) 
            return iRGenHelpers::LogCodeGenError("Trying to assign a primitive to an entity");
        if(not iRGenHelpers::isEqual(lvalueT->getPointerElementType(), rhsT->getPointerElementType()))
        ...
        AllocaInst* alloc = builder.CreateAlloca(lvalueT->getPointerElementType());
